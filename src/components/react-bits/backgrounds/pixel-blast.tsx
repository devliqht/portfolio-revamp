  ...
  useEffect(() => {
    ...
    if (!threeRef.current) mustReinit = true;
      ...
      for (const k of needsReinitKeys)
        if (
          prevConfigRef.current[k as keyof typeof cfg] !==
      ...
      if (noiseAmount > 0) {
        ...
        if (composer && composer.passes.length > 0)
          composer.passes.forEach(p => {
            if ('renderToScreen' in p) (p as EffectPass).renderToScreen = false;
          });
        ...
        if (composer) {
          ...
          composer.passes.forEach(p => {
            if (p instanceof EffectPass) {
              // access effects through public methods/properties if available
              // or update uniforms directly on known effects
              if (liquidEffect) {
                const timeUniform = liquidEffect.uniforms?.get('uTime');
                if (timeUniform) timeUniform.value = uniforms.uTime.value;
              }
