            ...
            {isOpen && (
                <div className="fixed inset-0 z-40">
                    ...
                    <div className={`relative z-10 flex flex-col justify-center items-start px-[5%] h-full transition-opacity duration-200 ${isClosing ? 'opacity-0' : 'opacity-100'}`}>
                        <nav className="space-y-4">
                            {navigationItems.map((item, index) => (
                                <div key={item.en} className={`animate-float-in-left `} style={{animationDelay: `${0.3 + index * 0.1}s`}}>
                                    <a href={`${item.link}`} className="block text-[5vw] font-dm-sans text-gray-800 dark:text-gray-100 font-semibold tracking-tight hover:cursor-pointer hover:text-transparent hover:[-webkit-text-stroke:2px_#111827] dark:hover:[-webkit-text-stroke:2px_#f3f4f6] hover:-skew-x-12 transition-transform duration-300" onClick={handleNavClick}>
                                        {item.en}
                                    </a>
                                    <p className="text-[2rem] font-baybayin text-gray-900 dark:text-gray-100 font-semibold tracking-tight opacity-70 -mt-4">
                                        {item.baybayin}
                                    </p>
                                </div>
                            ))}
                        ...
                        <div className={`absolute bottom-8 right-8 flex flex-col items-end space-y-4 transition-opacity duration-100 ${isClosing ? 'opacity-0' : 'opacity-100'} animate-float-in-right`} style={{animationDelay: '0.3s'}}>
                            <div className="flex flex-col items-end">
                                <ThemeToggle />
                            <h1 className="text-[4vw] font-dm-sans text-gray-900 dark:text-gray-100 font-thin tracking-tight">
                              {theme === 'dark' ? 'where are we hiding today?' : 'where are we heading today?'}
                            </h1>

                                <span className="text-gray-900 dark:text-gray-100 font-dm-sans font-semibold text-[3vw] tracking-tighter">Matt Cabarrubias</span>
                            </div>
