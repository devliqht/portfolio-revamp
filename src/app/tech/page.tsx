...
const TechSection: React.FC = () => {
  ...
  const updateScrollState = useCallback(() => {
    ...
    const currentGroupTranslateZ = groupBaseZ + progress * (groupEndZ - groupBaseZ);
    ...
    techIcons.forEach(icon => {
        ...
        if (progress >= flyStart) {
          flyProgressForVisibility = Math.min(1, (progress - flyStart) / iconFlyDuration);
        }
        
        const estimatedCurrentIconLocalZ = icon.z + flyProgressForVisibility * zFlyDistance;
        const estimatedFinalWorldZ = currentGroupTranslateZ + estimatedCurrentIconLocalZ;

        if (Math.abs(estimatedFinalWorldZ - visibilityCenterFocusZ) < visibilityFocusDepth / 2) {
            newVisibleIcons.push(icon.id);
        }
    });
    setVisibleIcons(newVisibleIcons);

  }, [totalIcons, groupBaseZ, groupEndZ, zFlyDistance, iconFlyStartFactor, iconFlyDuration, visibilityCenterFocusZ, visibilityFocusDepth]); 
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className="absolute inset-0 flex items-center justify-center">
            {[...Array(numLines)].map((_, i) => { 
              ...
              const length = 5 + Math.pow(Math.max(0.001, scrollProgress), 0.3) * 2500; 
              const lineOpacity = Math.min(0.4, Math.pow(Math.max(0.001, scrollProgress), 0.3) * 0.6); 
        ...
        >
          {techIcons.map((tech) => {
            ...
            if (scrollProgress >= flyStart) {
              iconFlyProgress = Math.min(1, (scrollProgress - flyStart) / iconFlyDuration);
            }
            ...
            if (distFromOpacityPeak < opacityHalfWidth) {
                opacity = 1 - Math.pow(distFromOpacityPeak / opacityHalfWidth, 1.2); 
            }
            if (finalWorldZ > opacityPeakZ + opacityHalfWidth * 1.5 || finalWorldZ < opacityPeakZ - opacityHalfWidth * 1.5) {
                opacity = 0;
            }
            opacity = Math.max(0, Math.min(1, opacity * 1.3)); 
            let additionalScale = minIconScale + (currentIconLocalZ - zForIconMinimumScale) / iconScaleSensitivity;
            additionalScale = Math.max(minIconScale, Math.min(additionalScale, maxIconScale));
            const xyClampedFactor = Math.max(0.01, (xyPerspectiveDivisor + currentIconLocalZ) / xyPerspectiveDivisor);
                ...
                <div className={`relative group`}>
                  <tech.Icon className={`w-16 h-16 md:w-20 md:h-20`} style={{ color: tech.color }} />
                    <div className="absolute inset-0 rounded-full" style={{ transform: 'scale(1.8)', zIndex: -1 }} />
                      <div className="absolute -bottom-8 left-1/2 transform -translate-x-1/2 text-black dark:text-white text-xs md:text-sm font-mono whitespace-nowrap" style={{ opacity: isActive ? 0.9 : Math.max(0, opacity * 0.7 - 0.1), fontSize: `${Math.max(0.5, Math.min(1.1, additionalScale * 0.65))}rem` }}>
                        {tech.name}
                    </div>
                </div>
              </div>
