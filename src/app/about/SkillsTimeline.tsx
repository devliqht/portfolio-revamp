...
const ComputerSkills = [
    ...
    { 
        ...
        description: "Developed low-level applications using C and C++, with a focus on memory management and manipulation, engaging of games such as Minecraft Bedrock Edition.",
        year: "2024"
    },
];
...
const CARD_INTERNAL_CONTENT_PADDING_TOTAL_VERTICAL_PX = CARD_PADDING_SINGLE_SIDE_PX * 2;
...
const MAX_CARD_CONTENT_BOX_HEIGHT_PX = CARD_HEADER_APPROX_HEIGHT_PX + MAX_CARD_DESCRIPTION_HEIGHT_PX + CARD_INTERNAL_CONTENT_PADDING_TOTAL_VERTICAL_PX;

// Total maximum outer height of the card (including its own border)
const MAX_CARD_OUTER_HEIGHT_PX = MAX_CARD_CONTENT_BOX_HEIGHT_PX + CARD_BORDER_TOTAL_VERTICAL_PX;
...
const CARD_ASSEMBLY_VERTICAL_OFFSET_PX = (MAX_CARD_OUTER_HEIGHT_PX + GAP_BETWEEN_CARD_AND_LINE_PX + FIXED_LINE_HEIGHT_PX) / 2;
...
export const SkillsTimeline: React.FC = () => {
    ...
    useEffect(() => {
        const calculateSizes = () => {
            if (timelineContentRef.current && stickyElementRef.current) {
                const calculatedMaxScroll = (ComputerSkills.length - 1) * timelineItemWidth;
                setMaxHorizontalScroll(Math.max(0, calculatedMaxScroll));
                const scrollSensitivityFactor = 1.5;
                const parallaxScrollHeight = Math.max(0, calculatedMaxScroll) * scrollSensitivityFactor; 
    ...
    useEffect(() => {
        const handleScroll = () => {
            if (!stickyContainerRef.current || !stickyElementRef.current || maxHorizontalScroll <= 0) {
            ...
            const parallaxEffectVerticalRange = stickyContainerRef.current.offsetHeight - window.innerHeight;
            if (parallaxEffectVerticalRange <= 0) {
                setScrollOffset(scrollTopRelativeToContainerStart > 0 ? maxHorizontalScroll : 0);
                return;
            }
            let currentHorizontalScroll = 0;
            if (scrollTopRelativeToContainerStart <= 0) currentHorizontalScroll = 0;
            else if (scrollTopRelativeToContainerStart >= parallaxEffectVerticalRange) currentHorizontalScroll = maxHorizontalScroll;
            else currentHorizontalScroll = (scrollTopRelativeToContainerStart / parallaxEffectVerticalRange) * maxHorizontalScroll;
    ...
    return (
        <section ref={stickyContainerRef} style={{ height: containerHeight, position: 'relative' }} id="skills">
            <div ref={stickyElementRef} className="sticky top-0 h-screen w-full overflow-hidden flex flex-col">
                ...
                <div className="relative w-full flex-grow h-[600px] overflow-hidden"> 
                    <div ref={timelineContentRef} className="relative flex items-center transition-transform duration-[25ms] ease-linear" style={{ transform: `translateX(-${scrollOffset}px)`, width: `${ComputerSkills.length * timelineItemWidth}px`, height: '100%', paddingLeft: `calc(50vw - ${timelineItemWidth / 2}px)`, paddingRight: `calc(50vw - ${timelineItemWidth / 2}px)`, }}>
                        <div className="absolute top-1/2 left-0 h-1 bg-gradient-to-r from-black via-neutral-600 to-black dark:from-white dark:via-neutral-400 dark:to-white transform -translate-y-1/2 z-0" style={{ width: '100%' }} />                        
                        {ComputerSkills.map((skill, index) => {
                            const isEven = index % 2 === 0;
                            const itemSlotLeftPosition = index * timelineItemWidth + (timelineItemWidth / 2);      
                            return (
                                <div key={index} className="absolute" style={{ left: `${itemSlotLeftPosition}px`, top: '50%', width: `${timelineItemWidth}px`, transform: 'translate(-50%, -50%)', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                                    <div className="absolute w-6 h-6 bg-black dark:bg-white rounded-full border-4 border-white dark:border-black z-20" style={{ left: '50%', top: '50%', transform: 'translate(-50%, -50%)' }} />
                                    <div className="relative flex flex-col items-center" style={{ width: `${cardWidth}px`, transform: isEven ? `translateY(-${CARD_ASSEMBLY_VERTICAL_OFFSET_PX}px)` : `translateY(${CARD_ASSEMBLY_VERTICAL_OFFSET_PX}px)` }}>
                                        <div className={`bg-white dark:bg-black rounded-lg p-6 w-full shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 hover:cursor-pointer ${isEven ? 'order-1' : 'order-2'}`}>
                                            <div className="flex items-center gap-4 mb-3">
                                                <div className="text-black dark:text-white">{skill.icon}</div>
                                                <div>
                                                    <h4 className="text-lg font-dm-sans font-semibold text-black dark:text-white">{skill.name}</h4>
                                                </div>
                                            </div>
                                            <p className="text-sm text-neutral-700 dark:text-neutral-300 leading-relaxed custom-scrollbar" style={{ maxHeight: `${MAX_CARD_DESCRIPTION_HEIGHT_PX}px`, overflowY: 'auto'}}>{skill.description}</p>
                                        </div>
                                        <div className={`w-1 bg-black dark:bg-white h-16 ${isEven ? 'order-2 mt-2' : 'order-1 mb-2'}`} />
                                    </div>
                                </div>
                            );
                        })}
                ...
                <div className="text-center py-4 flex-shrink-0 relative z-20">
                    <p className="text-sm text-neutral-600 dark:text-neutral-400 font-dm-sans">Scroll to explore the timeline →</p>
                </div>
            </div>
            <style jsx global>{`
                .custom-scrollbar::-webkit-scrollbar { width: 6px; }
                .custom-scrollbar::-webkit-scrollbar-track { background: transparent; }
                .custom-scrollbar::-webkit-scrollbar-thumb { background-color: rgba(128, 128, 128, 0.5); border-radius: 3px; }
                .custom-scrollbar { scrollbar-width: thin; scrollbar-color: rgba(128, 128, 128, 0.5) transparent; }
            `}</style>
        </section>
    );
};