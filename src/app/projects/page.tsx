...
export default function ProjectsSection() {
        ...
        if (projects_section) {
          ...
          if (
            ...
            if (new_project !== current_project) {
              ...
              setTimeout(() => set_is_transitioning(false), 250);
            }
          }
        }
      });
  ...
  const exact_project_index = useMemo(
    () => scroll_progress * projects.length,
    [scroll_progress]
  );
  ...
  return (
    <div
      className='min-h-screen relative bg-white dark:bg-neutral-900'
      id='projects'
      style={{
      ...
      <div className='sticky top-0 h-screen flex flex-col-reverse sm:md:flex-row'>
        <div className='w-full lg:w-2/5 flex items-center justify-center h-full p-6 lg:p-1 relative z-[40]'>
          <div className='max-w-lg w-full sm:md:p-8'>
            <div
              key={current_project}
              className={`transition-all duration-300 ease-out ${is_transitioning ? 'opacity-0 transform translate-y-4' : 'opacity-100 transform translate-y-0'}`}
            >
              ...
              <div className='flex flex-wrap gap-2 mb-6'>
                {projects[current_project]?.tags
                  .slice(0, 8)
                  .map((tag, index) => (
                    <span
                      key={index}
                      className='px-3 py-1 bg-neutral-200 dark:bg-neutral-800 text-neutral-700 dark:text-neutral-200 rounded-full tracking-tight text-xs lg:text-sm font-light transition-all duration-200 ease-out'
                      style={{
              ...
              <a
                ...
                className='inline-flex items-center w-fit gap-2 px-6 py-3 text-black border-[1px] border-black dark:border-white dark:text-white hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black rounded-lg transition-all duration-200 hover:scale-105'
              >
                View Project
                <svg
        ...
        <div className='w-full lg:w-3/5 relative overflow-hidden min-h-[50vh] lg:min-h-full'>
          <div className='absolute inset-0 flex items-center justify-center p-4'>
            <div className='mb-8 absolute top-[7vw]'></div>
            <div
            ...
            >
              {visible_projects.map(project => (
                <div
                  key={project.id}
                  className='absolute inset-0 will-change-transform transition-all duration-400 ease-out'
                  style={{
                ...
                >
                  <div className='relative w-full h-full'>
                    <div
                      className='w-full h-full overflow-hidden rounded-lg relative transition-all duration-300 ease-out hover:shadow-2xl hover:shadow-black/20'
                      style={{
