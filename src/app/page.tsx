    ...
    <main suppressHydrationWarning={true}>
      <div className={`relative min-h-screen flex text-white overflow-hidden ${isTypewriterComplete ? 'animated-dots-bg' : ''}`} id="landing">
        <div className="flex flex-col px-[5.8%] sm:md:px-[2.7%] top-12 sm:md:top-8 sm:md:pt-[4%] mt-8 z-10 relative h-fit transition">
          ...
          <h1 className="text-[14vw] font-dm-sans text-black dark:text-white font-semibold tracking-tight hover:cursor-pointer hover:text-transparent hover:[-webkit-text-stroke:2px_black] dark:hover:text-black dark:hover:[-webkit-text-stroke:2px_white] -translate-y-[4vw]"><Typewriter text="devliqht" delay={150} onComplete={handleTypewriterComplete}/></h1>
        </div>
        <div className="absolute bottom-0 px-[5%] pb-[4.7%] sm:md:left-0 right-0">
          <div className={`text-2xl sm:text-3xl md:text-[4vw] font-dm-sans text-gray-900 dark:text-gray-100 font-extralight tracking-widest mb-4 transition-opacity ${isTypewriterComplete ? 'animate-float-in-top' : ''} sm:md:text-left text-right opacity-0`}>full-stack</div>
          <h3 className={`text-2xl sm:md:text-4xl font-baybayin text-black sm:md:text-left text-right dark:text-white font-semibold tracking-tight ${isTypewriterComplete ? 'animate-float-in-top' : ''} opacity-0`}>devliqht</h3>
        </div>
        <div className="absolute right-0 top-8/12 scale-110 -translate-y-1/2 w-[400px] h-[560px] sm:h-[720px] md:h-[860px] group">
          <div className="absolute inset-0 -z-10">
            <div className={`absolute top-[48%] right-8 hover:scale-105 ${isTypewriterComplete ? 'animate-float-in-right' : ''} hover:translate-x-2 transition duration-300 hover:cursor-pointer opacity-0`}>
              <SiReact className="w-32 h-32 text-black dark:text-white hover:text-[#00D8FF] dark:hover:text-[#00D8FF] opacity-100" />
            </div>
            <div className={`absolute top-1/8 -left-0 -rotate-24 hover:scale-105 ${isTypewriterComplete ? 'animate-float-in-left' : ''} hover:-translate-x-2 transition duration-300 hover:cursor-pointer opacity-0`}>
              <SiVercel className="w-38 h-38 text-black dark:text-white hover:text-black dark:hover:text-black opacity-100" />
            </div>
            <div className={`absolute bottom-1/5 right-8 hover:scale-105 ${isTypewriterComplete ? 'animate-float-in-bottom' : ''} hover:translate-x-2 transition duration-300 hover:cursor-pointer opacity-0`}>
              <SiPhp className="w-38 h-38 text-black dark:text-white hover:text-[#777BB4] dark:hover:text-[#777BB4] opacity-100" />
            </div>
            <div className={`absolute top-1/7 right-9 rotate-11 hover:scale-105 ${isTypewriterComplete ? 'animate-float-in-top' : ''} hover:translate-x-2 transition duration-300 hover:cursor-pointer opacity-0`}>
              <SiLaravel className="w-42 h-42 text-black dark:text-white hover:text-[#fb503b] dark:hover:text-[#fb503b] opacity-100" />
            </div>
            <div className={`absolute top-[60%] -left-4 -rotate-16 hover:scale-105 ${isTypewriterComplete ? 'animate-float-in-bottom' : ''} hover:-translate-x-2 transition duration-300 hover:cursor-pointer opacity-0`}>
              <SiJavascript className="w-28 h-28 text-black dark:text-white hover:text-[#F7DF1E] dark:hover:text-[#F7DF1E] opacity-100" />
            </div>
            <div className={`absolute top-1/3 -left-0 hover:scale-105 ${isTypewriterComplete ? 'animate-float-in-left' : ''} hover:-translate-x-2 transition duration-300 hover:cursor-pointer opacity-0`}>
              <SiTailwindcss className="w-32 h-32 text-black dark:text-white hover:text-[#06B6D4] dark:hover:text-[#06B6D4] opacity-100" />
            </div>
          </div>
          
          <div className={`relative w-full h-full transition-all duration-500 ease-in-out pointer-events-none [mask:linear-gradient(to_bottom,black_0%,black_70%,transparent_100%)] sm:md:[mask:linear-gradient(to_bottom,black_0%,black_50%,rgba(0,0,0,0.6)_70%,transparent_80%)] ${isTypewriterComplete ? 'opacity-100' : 'opacity-0'}`}>
            <Image
